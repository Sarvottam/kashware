  ...
  describe('/Patch patchRequest', () => {
    ...
    it('it should add address key-value pair to the object {"name":"Sarvo","roll" :2}', (done) => {
      chai
        ...
        .set('Authorization', jwtToken)
        .send({
          reqObject: { name: 'Sarvo', roll: 2 },
          patchObject: [{ op: 'add', path: '/address', value: 'Mumbai' }],
        })
        .end((err, res) => {
          console.log('', res.body);
          res.should.have.status(200);
          res.body.result.should.be.eql({
            data: { name: 'Sarvo', roll: 2, address: 'Mumbai' },
          });
          done();
        });
    });
    ...
    it('it should fail while removing address key-value pair to the object as it dont  exist', (done) => {
      chai
        ...
        .send({
          reqObject: { name: 'Sarvo', roll: 2 },
          patchObject: [{ op: 'remove', path: '/address' }],
        })
        .end((err, res) => {
          res.should.have.status(400);
    ...
    it('it should  be able to remove roll (key-value) from the object', (done) => {
      chai
        ...
        .send({
          reqObject: { name: 'Sarvo', roll: 2 },
          patchObject: [{ op: 'remove', path: '/roll' }],
        })
        .end((err, res) => {
          res.should.have.status(200);
